...
# Default LLM model
LMSTUDIO_MODEL = os.getenv("LMSTUDIO_MODEL", "lmstudio-community/gemma-3-1b-it-GGUF")
...
def download_file(rel_path):
    ...
    if not abs_path.exists():
        ...
        return redirect(url_for("index"))
    return send_from_directory(UPLOAD_FOLDER, rel_path, as_attachment=True)
...
def media_file(rel_path):
    ...
    return send_from_directory(UPLOAD_FOLDER, rel_path, as_attachment=False)
...
def preview_file(rel_path):
    abs_path = Path(app.config.get('UPLOAD_FOLDER')) / rel_path
    if not abs_path.exists():
        flash('Файл не найден.', 'danger')
        return redirect(url_for('index'))
    ...
    # Пробуем найти запись File, чтобы получить id для ссылки «Подробнее»
    f = File.query.filter_by(rel_path=str(rel_path)).first()
    ...
    if f and (is_audio or (f.material_type or '') == 'audio'):
        ...
        audio_url = url_for('media_file', rel_path=rel_path)
        try:
    ...
    if request.args.get('embedded') in ('1', 'true', 'yes'):
        return render_template('preview_embedded.html', filename=rel_path, is_pdf=is_pdf, is_text=is_text, is_audio=is_audio, content=content, thumbnail_url=thumbnail_url, file_id=file_id, abstract=abstract, audio_url=audio_url, duration=duration)
    return render_template('preview.html', filename=rel_path, is_pdf=is_pdf, is_text=is_text, is_audio=is_audio, content=content, thumbnail_url=thumbnail_url, file_id=file_id, abstract=abstract, audio_url=audio_url, duration=duration)
...
def call_lmstudio_keywords(text: str, filename: str):
    ...
    system = (
        "Ты извлекаешь ключевые слова из стенограммы аудио. "
        "Верни только JSON-массив строк на русском: [\"ключ1\", \"ключ2\", ...]. "
        "Без пояснений, не более 12 слов/фраз."
    )
    user = f"Файл: {filename}\nСтенограмма:\n{text}"
    try:
        base = LMSTUDIO_API_BASE.rstrip('/')
...
def call_lmstudio_for_metadata(text: str, filename: str):
    ...
    allowed_types = [
        "dissertation", "dissertation_abstract", "article", "textbook",
        "monograph", "report", "patent", "presentation",
        "proceedings", "standard", "note", "document"
    ]
    system = (
        "Ты помощник по каталогизации научных материалов. "
        "Твоя задача: определить тип материала из набора: " + ", ".join(allowed_types) + ". "
        "Если подходит несколько — выбери наиболее вероятный. "
        "Верни ТОЛЬКО валидный JSON без пояснений. "
        "Ключи: material_type (одно из перечисленных), title, author, year, advisor, "
        "keywords (array), novelty (string), literature (array), "
        "organizations (array), classification (array). "
        "Если данных нет — ставь пустые строки или пустой массив."
    )
    user = f"Файл: {filename}\nФрагмент текста:\n{text}"
...
def scan():
    ...
    for path in root.rglob("*"):
        ...
        # Извлечение текста (пока для PDF)
        text_excerpt = ""
        if do_extract and ext == ".pdf":
            text_excerpt = extract_text_pdf(path, limit_chars=40000)
            file_obj.text_excerpt = text_excerpt[:40000]

        # Извлечение текста для поддерживаемых форматов
        text_excerpt = ""
        if do_extract:
            if ext == ".pdf":
                text_excerpt = extract_text_pdf(path, limit_chars=40000)
        ...
        # Определим тип, если пустой
        if not file_obj.material_type:
            file_obj.material_type = guess_material_type(ext, text_excerpt, filename)
        ...
        if use_llm and (text_excerpt or ext in {'.txt', '.md'}):
            ...
            if meta:
                file_obj.material_type = normalize_material_type(meta.get("material_type")) or file_obj.material_type
                file_obj.title = meta.get("title") or file_obj.title
                file_obj.author = meta.get("author") or file_obj.author
                file_obj.year = meta.get("year") or file_obj.year
                file_obj.advisor = meta.get("advisor") or file_obj.advisor
...
def settings():
    ...
    global TRANSCRIBE_ENABLED, TRANSCRIBE_BACKEND, TRANSCRIBE_MODEL_PATH, TRANSCRIBE_LANGUAGE, SUMMARIZE_AUDIO, AUDIO_KEYWORDS_LLM
    if request.method == "POST":
        SCAN_ROOT = Path(request.form.get("scan_root") or SCAN_ROOT)
        # align upload folder with scan root so imports land in the scanned root
...
def diag_transcribe():
    ...
    try:
        ...
        # Выполнить транскрибацию при наличии файла и разрешённых настройках
    ...
    try:
        ...
        if use_llm and (text_excerpt or ext in {'.txt', '.md'}):
            ...
            if meta:
                ...
                f.year = meta.get("year") or f.year
                f.advisor = meta.get("advisor") or f.advisor
                kws = meta.get("keywords") or []
    ...
    with app.app_context():
        try:
            ...
            for idx, path in enumerate(file_list, start=1):
                ...
                # LLM enrichment (optional, can be slow)
                if use_llm and (text_excerpt or ext in {'.txt', '.md'}):
                    ...
                    for key in ("literature", "organizations", "classification"):
                        ...
                        if isinstance(val, list) and val:
                            db.session.flush()
                            upsert_tag(file_obj, key, "; ".join([str(x) for x in val]))
                    # Summarize audio transcript into abstract
                    if ext in AUDIO_EXTS and SUMMARIZE_AUDIO and (file_obj.text_excerpt or ''):
                        ...
                        # Повторный прогон типо-зависимых тегов (вдруг сменился тип)
                        try:
                            ttags = extract_tags_for_type(file_obj.material_type or '', text_excerpt or '', filename)
                            if ttags:
                                db.session.flush()
